...
const defaultRelatedServices: RelatedService[] = [
  ...
  { icon: Zap, label: 'Gate Openers', href: '/automatic-gates' },
...
export function AuthoritySection({
  ...
  return (
    <section className="py-24 md:py-32 bg-[hsl(220,35%,12%)] relative overflow-hidden">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start mb-20">
            ...
            {images.map((img, index) => (
              ...
              >
                ...
                <img
                  ...
                  className="relative w-full h-auto max-h-[400px] object-cover rounded-3xl shadow-2xl shadow-black/50 border border-white/10"
                />
              </div>
            ))}
          </div>
          ...
          <div className="space-y-5 order-2">
            {trustCards.map((card, index) => (
              <div
                key={index}
                className="group p-7 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 shadow-xl shadow-black/30 hover:bg-white/10 hover:border-primary/30 hover:-translate-y-1 hover:shadow-2xl hover:shadow-primary/10 transition-all duration-300"
              >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {relatedServices.map((service, index) => (
              <Link
                key={index}
                to={service.href}
                className="group p-5 rounded-xl bg-white/5 border border-white/10 text-center hover:bg-primary/10 hover:border-primary/40 hover:-translate-y-1 transition-all duration-300"
              >
