...
function StarRating({
  ...
  return <div className="flex gap-0.5">
      {[...Array(5)].map((_, i) => <Star key={i} className={`w-5 h-5 ${i < rating ? 'fill-primary text-primary' : 'text-muted'}`} />)}
    </div>;
}
...
export function ReviewWidget() {
  ...
  return <section id="reviews" className="py-20 md:py-28 border-t border-border">
      <div className="container-main">
        {/* Summary Header - Bob's Style */}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {reviews.map((review, index) => <div key={index} className={`card-premium p-6 rounded-2xl transition-all duration-500 ${index === activeIndex ? 'ring-2 ring-primary/50 scale-[1.02]' : ''}`}>
              <div className="flex items-start gap-4 mb-4">
                <img src={review.image} alt={review.name} className="w-14 h-14 rounded-full object-cover border-2 border-primary/30" />
                <div>
        ...
        <div className="flex justify-center gap-2 mt-8 md:hidden">
          {reviews.map((_, index) => <button key={index} onClick={() => setActiveIndex(index)} className={`w-2.5 h-2.5 rounded-full transition-all ${index === activeIndex ? 'w-8 bg-primary' : 'bg-muted'}`} />)}
        </div>
      </div>
    </section>;
}