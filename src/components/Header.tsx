...
const navItems = [
  ...
  { 
    ...
    href: '#',
    children: [
      { label: 'Automatic Gates', href: '/automatic-gates' },
      { label: 'Access Control', href: '/access-control' },
      { label: 'Gate Repair', href: '/gate-repair' },
      { label: 'Driveway Gates', href: '/driveway-gates' },
            ...
            {navItems.map((item) => (
              item.children ? (
                <div key={item.label} className="relative group">
                  <button 
                    className="flex items-center gap-1 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors uppercase tracking-wide"
                  >
                    {item.label}
                    ...
                    <div className="bg-card border border-border rounded-xl shadow-xl py-2 min-w-[200px]">
                      {item.children.map((child) => (
                        <Link
                          key={child.href}
                          to={child.href}
                          className={`block px-4 py-2.5 text-sm font-medium transition-colors hover:bg-primary/10 hover:text-primary ${
                ...
                item.children ? (
                  <div key={item.label}>
                    <button
                      ...
                      className="flex items-center justify-between w-full px-4 py-3 text-sm font-medium text-foreground uppercase tracking-wide"
                    >
                      {item.label}
                  ...
                  <Link
                    ...
                    to={item.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className="block px-4 py-3 text-sm font-medium text-foreground uppercase tracking-wide hover:text-primary transition-colors"
                  >
                    {item.label}
                  </Link>
                )
