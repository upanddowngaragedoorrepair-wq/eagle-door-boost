...
export function BrandMarquee({
  ...
  return <section className="py-16 md:py-20 bg-[hsl(var(--navy-deep))] border-t border-border overflow-hidden bg-yellow-50 text-primary-foreground">
      <div className="container-main mb-10">
        <p className="text-sm tracking-wide uppercase font-sans font-bold text-primary-foreground text-center md:text-2xl">
      ...
      <div className="relative" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
        ...
        {/* Scrolling Track */}
        <div ref={containerRef} className="flex items-center gap-6 md:gap-10" style={{
        animation: `marquee-scroll ${speed}s linear infinite`,
        animationPlayState: isPaused ? 'paused' : 'running',
        width: 'fit-content'
      }}>
          {duplicatedLogos.map((logo, index) => <div key={`${logo.name}-${index}`} className="flex-shrink-0 group">
              <div className="w-32 h-20 md:w-44 md:h-28 lg:w-52 lg:h-32 rounded-xl bg-card/60 border border-border flex items-center justify-center px-4 transition-all duration-300 group-hover:scale-105 group-hover:border-primary/40 group-hover:shadow-lg group-hover:shadow-primary/10 cursor-pointer">
                {logo.image ? <img src={logo.image} alt={`${logo.name} logo`} className="max-w-full max-h-full object-contain filter brightness-90 group-hover:brightness-100 transition-all duration-300" draggable={false} /> : <div className="flex flex-col items-center gap-2">
